...
4. 로컬 GPU에서 `python3 -m src.pipeline.prepare_worker --mode auto --loop`를 실행해 서버가 enqueue한 신규 날짜 prepare 작업을 소비하고, 성공한 논문은 같은 워커에서 임베딩까지 이어서 처리한다.
...
python3 -m src.pipeline.prepare_worker --mode auto --max-jobs-per-run 1 --embed-max-chunks 200 --loop --sleep-seconds 120
