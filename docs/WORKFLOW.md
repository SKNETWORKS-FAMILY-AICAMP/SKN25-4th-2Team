...
- `src.pipeline.prepare_worker`로 날짜 배치 단위 prepare 실행
...
4. 로컬 GPU에서 `python3 -m src.pipeline.prepare_worker --mode auto --loop`를 실행해 서버가 enqueue한 신규 날짜 prepare 작업을 소비한다.
...
python3 -m src.pipeline.prepare_worker --mode auto --max-jobs-per-run 1 --loop --sleep-seconds 120
