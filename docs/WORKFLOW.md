...
이 문서는 ArXplore 베타를 5인 팀이 병렬로 구현하고 통합할 때 따를 작업 흐름을 정리한 운영 기준 문서이다. 환경 설정 자체는 [TEAM_SETUP.md](./TEAM_SETUP.md)를 기준으로 하고, 본 문서는 "어떤 순서로 구현하고, 어떤 시점에 통합하며, 어떤 기준으로 완료를 판단할 것인가"를 설명한다.
...
- 공통 용어는 `TopicDocument`, `collect_papers`, `prepare_papers`, `embed_papers`, `analyze_topics`를 사용한다.
...
`TopicDocument` 계약은 `src/core/models.py`를 기준으로 유지한다. AI 도구를 사용하더라도 이 계약은 임의로 수정하지 않는 것을 원칙으로 한다. AI 작업 규칙은 [AGENTS.md](./AGENTS.md)를 따른다.
...
- 수집 담당: `src/integrations/paper_search.py`, `src/integrations/raw_store.py`, `src/pipeline/collect_papers.py`, `src/pipeline/prepare_papers.py`
- 저장 담당: `src/integrations/paper_repository.py`, `src/integrations/topic_repository.py`, `src/integrations/embedding_client.py`, `src/integrations/vector_repository.py`, DB 스키마
...
3. 임베딩과 토픽 매핑을 저장한다.
4. 토픽별 논문 묶음을 읽어 `TopicDocument`를 생성한다.
5. 논문 청크 검색 경로를 연결한다.
6. 생성된 문서를 저장하고 UI에서 조회한다.
7. 검색 응답을 근거 논문과 함께 노출한다.
...
수집 담당자는 "논문 원문 메타데이터가 얼마나 안정적으로 들어오는가"를 중심으로 작업한다.

1. HF Daily Papers API 호출 결과를 수집한다.
2. 원본을 MongoDB에 저장한다.
3. arXiv 메타데이터 보강 결과를 검증한다.
...
2. 저장 함수와 조회 함수를 만든다.
3. 토픽, 문서, 근거 논문 매핑 구조를 확정한다.
4. UI와 LLM 담당이 재사용할 읽기 함수를 제공한다.

### LLM · RAG 담당

LLM 담당자는 "논문이 어떤 규칙으로 문서가 되는가"와 "검색 결과가 어떤 규칙으로 답변이 되는가"를 책임진다.

1. 샘플 논문 묶음을 준비한다.
2. 프롬프트를 수정한다.
3. LangSmith trace로 결과를 비교한다.
4. `TopicDocument` 계약에 맞게 체인 출력을 안정화한다.
...
- `prepare_papers`: arXiv 메타데이터 보강 및 정제
- `embed_papers`: 임베딩 및 토픽 묶기
- `analyze_topics`: 토픽 문서 생성

문서 생성 품질 검토는 특히 `stage=analyze_topics`를 중심으로 수행한다.
...
2. PostgreSQL에 정제 논문과 청크가 저장되는지 확인한다.
3. 임베딩과 토픽 매핑이 저장되는지 확인한다.
4. `TopicDocument`가 생성되는지 확인한다.
5. 문서 저장 결과를 다시 조회할 수 있는지 확인한다.
6. Streamlit에서 검색 영역, 카드 영역, 상세 문서가 정상 렌더링되는지 확인한다.
7. 검색 결과와 근거 논문이 함께 노출되는지 확인한다.
