...
- `run_prepare_worker.py`로 날짜 배치 단위 prepare 실행
...
- `arxplore_orchestrate_ingestion`: 수동 트리거용 상위 DAG이다. `collect -> backfill -> enrich`를 순차 실행하고, `backfill` 실패 시에도 `enrich`는 `all_done` 규칙으로 계속 실행한다.
...
`arxplore_orchestrate_ingestion`
...
4. 로컬 GPU에서 `python3 scripts/run_prepare_worker.py --mode auto --loop`를 실행해 신규 수집분 prepare를 자동 추적한다.
5. 과거 날짜를 파싱할 때만 `--mode backfill`을 수동 실행한다.
6. `notebooks/inspect_ingestion.ipynb`의 `quality_issues`와 chunk 출력 셀로 watchlist를 확인한다.
7. 반복되는 오탐 패턴만 다시 파서 규칙으로 보정한다.
8. 파서 기준이 크게 바뀌면 PostgreSQL 정제층을 초기화한 뒤 다시 적재한다.
...
python3 scripts/run_prepare_worker.py --mode auto --loop --sleep-seconds 120
python3 scripts/run_prepare_worker.py --mode backfill --batch-days 3 --loop --sleep-seconds 120
```
