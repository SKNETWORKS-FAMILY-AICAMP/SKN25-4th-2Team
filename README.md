...
- Streamlit 기반 검색, 카드 그리드, 상세 문서, 에이전트 채팅 UI
...
- **Streaming** `stream_mode="messages"`로 실시간 타이핑 응답을 Streamlit UI에 전달
...
| UI | Streamlit views | 논문 리스트 / 상세 / 에이전트 채팅 페이지 수동 회귀 |
...
- LangGraph React Agent가 도구 호출과 스트리밍 응답을 안정적으로 수행, Streamlit UI에서 실시간 타이핑으로 노출됨
...
```text
app/                    Streamlit 라우터 (main.py)
app/views/              페이지 뷰 (list, detail, agent_chat)
app/components/         공통 UI 컴포넌트와 네비게이션
